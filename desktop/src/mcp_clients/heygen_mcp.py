...
class HeyGenMCP:
	...
	def list_avatars(self) -> Any:
		for c in ("/avatars","/v1/avatars","/api/avatars"):
			try: return self._request("GET", c)
			except Exception: continue
		raise RuntimeError("Failed to list avatars")
	...
	def generate_video_from_text(self, text: str, avatar: Optional[str] = None, style: Optional[str] = None, video_opts: Optional[Dict] = None) -> Dict:
		...
		if video_opts: payload["video"]=video_opts
		for c in ("/generate","/v1/generate","/video/generate"):
	...
	def get_status(self, job_id: str) -> Any:
		for c in (f"/status/{parse.quote(job_id)}", f"/v1/status/{parse.quote(job_id)}", f"/jobs/{parse.quote(job_id)}"):
			try: return self._request("GET", c)
			except Exception: continue
		raise RuntimeError("Failed to get job status")

	def download_video(self, video_id: str, save_path: Optional[str] = None) -> bytes:
		for c in (f"/download/{parse.quote(video_id)}", f"/videos/{parse.quote(video_id)}/download"):
			try:
